...
def get_os_info(target): 
	...
	# Print it all pretty like. 100% likely a more Pythonic way to do this but whatever.
...
def get_mem_info(target):
	...
	# we *could* just throw that in here, but as this re-write builds out we may 
	# eventually break down the check_mem function so that we can selectively pull targets
...
def get_ip_info(target):
	...
	for key in sorted(ip_ints):
		...
		if 'eth' in value[0] or 'em' in value[0]:
			print '\t' + colors.WHITE + '{:<15s}'.format(value[0]) + colors.ENDC + '{:^36s}{:<16s} {:>11}'.format(value[1],value[2],value[3])
		...
		#awayfar's function here. Keeping this for reference in case I need to do 
		# column based output again - using this function would be better
		# but as of right now, this is the only thing that uses columns
		
		
		#offset0 = getoffset(15, value[0], format_color(value[0])) 
		#offset1 = getoffset(15, value[1], format_color(value[1]))
		#offset2 = getoffset(15, value[2], format_color(value[2]))
		#print '\t{:<{foffset0}s}{:^{foffset1}s}{:>{foffset2}}'.format(format_color(value[0]),format_color(value[1]),format_color(value[2]), foffset0=offset0, foffset1=offset1, foffset2=offset2)			
...
def get_netdev_info(target):
	...
	for item in sorted(dev_info):
		...
		+ tab + ' %9.2f' % round((int(value[1]) / (1000)), 2) + ' k' + tab + '{:>4}'.format(value[2])\
		+ tab + '{:>6}'.format(value[3]) + tab + '%7.2f' % round(int(value[8]) / (1024 * 1024 * 1024), 2)\
		+ tab + '  %8.2f' % round((int(value[9]) / (1000 )), 2) + ' k' + tab + '{:>4}'.format(value[10])\
...
def get_main(os, mem, kdump, cpu, sysctl, ip, bonding, test, netdev, rhev, target):
	if os:
		get_os_info(target)
	if mem:
		get_mem_info(target)
...
, args.bonding, args.test, args.netdev, args.rhev, target)

>>>>>>> 59dae46494ae42648ea1016b204e4b416c371eeb
